import rewardsPenaltiesApi from '@/src/service/rewardsPenaltiesService';

export default {
    namespace: 'rewardsPunishmentsList',
    state: {
        // 个人
        tabList1: [],
        tabDatas1: [],
    ...
    effects: {
        *add({ payload, callback }, { call, put }) {
            const res = yield call(rewardsPenaltiesApi.add, payload)
        ...
        *getDetail({ payload }, { call, put }) {
            const res = yield call(rewardsPenaltiesApi.getDetail, payload)
        },
        *getOrgList({ payload, callback }, { call, put }) {
            const res = yield call(rewardsPenaltiesApi.getOrgList, payload)
            
            if(callback) {
                callback(res)
            }
        },
        *getRewardsList({ payload, callback }, { call, put }) {
            const res = yield call(rewardsPenaltiesApi.getRewardsList, payload)
            const { code, data } = res
        ...
        *getSysDictListByTypeCode({ payload, callback }, { call, put }) {
            const res = yield [
                call(rewardsPenaltiesApi.getSysDictListByTypeCode, {typeCode: '0'}),
                call(rewardsPenaltiesApi.getSysDictListByTypeCode, {typeCode: '1'}),
                call(rewardsPenaltiesApi.getSysDictListByTypeCode, {typeCode: '2'}),
                call(rewardsPenaltiesApi.getSysDictListByTypeCode, {typeCode: '3'}),
            ]
