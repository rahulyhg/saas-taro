...
class AddActTypesForm extends Component<AddActTypesFormProps, AddActTypesFormState> {
    ...
    saveEvt() {
        ...
        if(_lodash.isEmpty(optionItem.name.trim())){
            Taro.showToast({
                title:'提示语不能为空',
                icon:'none'
            })
        ...
        return (
            <View className='addActTypesForm-wrap'>
                <View className="posrel formList">
                    <AtInput className="label labelInput" value={optionItem.name} type='text' maxLength={50} onChange={this.inputTitle}
                        placeholder={`请输入${placeholder}提示语`} focus border={false} />
