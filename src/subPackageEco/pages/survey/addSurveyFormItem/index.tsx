...
class AddTypesForm extends Component<AddFormItemProps, AddFormItemState> {
    ...
    saveEvt() {
        ...
        if (_lodash.isEmpty(question.title.trim())) {
            Taro.showToast({
                title: '标题不能为空',
                icon: 'none'
            })
        ...
        if ((question.type == '0' || question.type == '1') && question.answerList.length <= 0) {
            Taro.showToast({
                title: '至少新增一个选项',
                icon: 'none'
            })
        ...
        if ((question.type == '0' || question.type == '1') && question.answerList.length > 0) {
            const list = question.answerList;
            let flag = true;
            for (let i = 0; i < list.length; i++) {
                if (_lodash.isEmpty(list[i].name.trim())) {
                    Taro.showToast({
                        title: '选项名称不能为空',
                        icon: 'none'
                    })
        ...
        return (
            <View className='addFormItem-wrap'>
                ...
                <View className="posrel formList">
                    <Input className="label labelInput" value={question.title} type='text' maxLength={50} onInput={this.inputTitle.bind(this)}
                ...
                </View>
                {
                    question.answerList && question.answerList.map((element, index) => {
                        return (
                            element.type === '0' ? (
                                <View className="posrel formList">
                                    <Input className="label labelInput" value={element.name} type='text' maxLength={50} onInput={this.inputOptionName.bind(this, index)}
                                    ...
                                    <View className="posrel formList">
                                        <Input className="label labelInput" value={`其他__`} type='text' maxLength={50} onInput={this.inputOptionName.bind(this, 0)}
