...
import { View, Text, Switch, Block } from '@tarojs/components'
...
class RedBag extends Component<RedBagProps, RedBagState> {
    ...
    constructor(props: RedBagProps) {
        ...
        this.state = {
            switchValue: false,
            formList: [
                { label: '红包数量', placeholder: '请输入', inputValue: '' },
                { label: '单个金额（元）', placeholder: '请输入', inputValue: '' },
                { label: '合计金额（元）', placeholder: '请输入', inputValue: '', isShowResult: true, resultValue: 0 },
                { label: '支付金额合计（元）', placeholder: '请输入', inputValue: '', isShowResult: true, resultTotalValue: 0 },
            ],
        }
    }
    ...
    submitSurvey = () => {
        ...
        dispatch({
            ...
            payload: {
                ...
                redTotalNum: Number(formList[0].inputValue),
                redMoney: Number(formList[1].inputValue),
                redTotal: Number(formList[0].inputValue) * Number(formList[1].inputValue),
                redPayTotal: (Math.ceil(Number(formList[0].inputValue) * Number(formList[1].inputValue) * 1.006 * 1000) / 1000).toFixed(2)
            }
        })
