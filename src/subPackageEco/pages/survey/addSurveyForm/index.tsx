...
class AddVoteForm extends Component<AddSurveyFormProps, AddSurveyFormState> {
    ...
    handleNextStep(){
        if((this.state.atcList && this.state.atcList.length > 0) || (this.state.questions && this.state.questions.length > 0)){
            Taro.navigateTo({
                            ...
                            return (
                                <View className='option-item' onClick={this.goEdit.bind(this, index, item.type)}>
                                    <View className='title'>
                                        <Text>{item.type == '0' ? '单选题' : (item.type == '1' ? '多选题' : '问答题')}</Text>{index + 1}.{item.title}
