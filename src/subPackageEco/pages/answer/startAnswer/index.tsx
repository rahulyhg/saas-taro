...
class AnswerDetail extends Component<AnswerProps, AnswerState> {
    ...
    componentWillUnmount() {
        Taro.switchTab({ url: '/pages/eco/home/index' });
    ...
    toNext(curIndex) {
        ...
        if(customValues[curIndex - 1].answerValueList.length <= 0) {
            Taro.showToast({
                title: '本题还未作答',
                icon: 'none',
                duration: 2000
    ...
    render() {
        const {showCover, customList, curIndex, result} = this.state;
        ...
        return (
            <View className="answer-wrap">
                        ...
                        return (
                            <View className='container' style={`display: ${ index== curIndex ? 'block': 'none'}`} key={ question.id }>
                                ...
                                <View className="title">
                                    <Text className="option-type">{ question.type == 'radio' ? '单选' : '多选' }</Text>
                ...
                <View className={`${showCover ? 'sysShow' : 'sysHide'}`}>
                    <View className="opacityCover" onClick={ this.cancelAddType.bind(this) }></View>
                    <View className="popup">
                        <Image src={huizhang} mode='widthFix' />
                        <View className='title'>{result.num > 0 ? `恭喜你答对${result.num}题` : '继续努力哟'}</View>
